...
public:
    ...
    void drawPoints(std::span<const vec2> pos, std::span<const float> radius, std::span<const Color> color);
    ...
    void drawPoints(const vec2* pos, const float* radius, const Color* color, size_t count, size_t stride_in_bytes = 0);
    ...
    */
    ...
    bool sliderVec2(std::string_view label, vec2& value, float min, float max);
    ...
    */
    bool colorPicker(std::string_view label, Color& color);
    ...
    */
    ...
    /**
    * Draws a checkbox with the provided \p label. If the user modifies the value through interaction,
    * this function returns \c true, otherwise, \c false is returned.
    *
